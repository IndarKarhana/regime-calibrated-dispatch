...
\author{
  \IEEEauthorblockN{Indar Kumar}
  \IEEEauthorblockA{Independent Researcher\\
    \texttt{github.com/IndarKarhana}}
  \and
  \IEEEauthorblockN{Akanksha Tiwari}
  \IEEEauthorblockA{Independent Researcher}
}
...
Cohen's $d = 7.5$--$29.9$ across scenarios). The improvement extends to the tail: P95 wait drops \SI{37.6}{\percent}
and the Gini coefficient of wait times improves from 0.441 to 0.409
(\SI{7.3}{\percent} relative). The two contributions compose
multiplicatively and are independently validated: calibration provides
\SI{16.9}{\percent} reduction; LP repositioning adds a further
\SI{15.5}{\percent}. The approach requires no training, is
...
demand prior---we use ``calibrated'' in the operational sense of adjusting
a demand estimate using retrieved historical analogues, distinct from the
statistical sense of probability calibration. For each time bin $t$:
...
testable; we assess it indirectly in \Cref{sec:ablation}, where the
similarity component ablation shows that distributional metrics---which
most directly measure demand-shape proximity---drive the majority of
matching quality. Direct per-regime consistency testing is left to
future work.
...
available drivers in $O(n^2 m)$ time for the rectangular case
(our implementation pads to a square matrix, yielding
$O(\max(n,m)^3)$). This is \emph{within-batch optimal} but myopic: it does not account for future requests that may
...
2024 (5.2 million trips after filtering to Manhattan). Provides diverse
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    \toprule
    Scenario & Replay+Batch (s) & Cal+LP (s) & Improv. & 95\% CI & Cohen's $d$ \\
    \midrule
...
show consistent, large-magnitude improvements, with a grand mean
reduction of \SI{31.1}{\percent} in mean wait time.
...
The two contributions compose multiplicatively.
Relative to the replay+batch baseline (\Cref{tab:extended_baselines}),
calibration alone reduces mean wait by \SI{16.9}{\percent}
(from \SI{122.4}{\second} to \SI{101.7}{\second}). Adding LP
reposit\-ioning further reduces the \emph{remaining} wait by
\SI{15.5}{\percent} (from \SI{101.7}{\second} to \SI{85.9}{\second}).
The compound reduction is $1-(1-0.169)(1-0.155)=29.8\%$, consistent
with the \SI{31.1}{\percent} in the 5-seed main experiment
(\Cref{tab:main_results}). \Cref{fig:decomposition} visualizes
this decomposition.
...
Using the NYC-built regime library on Chicago TNP data (no retraining
or local calibration), Cal+LP achieves \SI{23.3}{\percent} average
wait reduction across 3~scenarios: \SI{17.2}{\percent} (weekday AM,
\SI{260}{\second}$\to$\SI{215}{\second}), \SI{30.0}{\percent}
(weekday PM, \SI{316}{\second}$\to$\SI{221}{\second}), and
\SI{22.8}{\percent} (weekend midday,
\SI{226}{\second}$\to$\SI{174}{\second}). The higher absolute
waits reflect Chicago's larger area and lower taxi density. Demand
\emph{shapes}---rush-hour peaks, weekend troughs---are qualitatively
similar across cities, enabling regime transfer without retraining.
...
by convention, $d > 0.8$ is already ``large.'' (These extreme values
reflect the low seed-to-seed variance of a near-deterministic
simulator, not a clinical-scale effect.)
...
(\SI{3}{\percent}) due to fleet under-provisioning (88\% busy). OSRM
travel times in Manhattan average ${\sim}1.45\times$ the Haversine
baseline at 30\,km/h, reflecting one-way streets, grid detours, and
turn penalties. Scaling fleet by $1.45\times$ compensates for this
effective-capacity gap and recovers the improvement to
\SI{25.7}{\percent}, confirming that the routing engine amplifies
travel times but the calibration gain is preserved
(\Cref{fig:osrm_recovery}).
...
automated weight selection. We retain the full 6-metric ensemble as
the default because (i)~the event and temporal components target edge
cases (holidays, anomalous surges) that may be under-represented in
the current 8-scenario test set, and (ii)~distributional-only matching
risks overfitting to demand-shape similarity when calendar context
(e.g., weekday vs.\ weekend) is informative. Reporting the
4-metric result alongside the 6-metric default gives practitioners
...
(\Cref{tab:batchwindow}). At $W=30$\,s the improvement is
\SI{35.7}{\percent}; at $W=120$\,s it is \SI{28.6}{\percent}. The
default $W=60$\,s (\SI{32.2}{\percent}) balances computational
cost and matching quality.\footnote{The simulator's 30\,s time step
imposes a floor: any $W < 30$\,s batches every step, so we omit
sub-step windows.} Notably, both the baseline and our method
...
LR/entropy decay. We note that 2,000 episodes is modest by deep RL
standards; a dedicated effort with more advanced architectures
(e.g., attention-based multi-agent methods) could narrow the gap.
Nonetheless, the result supports the LP approach: when the objective
has structure (linear cost, flow constraints), optimization provides
strong performance without the sample and compute costs of learning.
...
average wait reduction with all scenarios showing large and consistent
improvement,
...
% NOTE: For double-blind submission, anonymize this self-citation
\bibitem{kumar2026rg}
...
[Online]. Available: \url{https://eng.uber.com/h3/}
